                                        ...
                                        <tbody id="myTable">
                                           @foreach($notifications as $row) 
                                            <tr>
                                                ...
                                                <td>
                                                    <h6>{{$row->intake_name}}</h6><small class="text-muted">{{$row->description}}</small><br><br><small class="text-muted">{{$row->time}}</small></td>
                                                <td><small class="text-muted">{{$row->date}}<br><br><br><br></small></td>
                                              
                                           <!--  <td>
                                                <a href="{{url('rejectpayrollreq/'.$row->intakeco_id)}}">Cancel</a>
...
<div class="tab-pane fade show fade" id="nav-Payroll" role="tabpanel" aria-labelledby="nav-Payroll-tab">
<div class="table-responsive">
<table class="table table-hover earning-box">
                                        <tbody id="myTable">
                                           @foreach($payrollrequest as $row) 
                                            <tr>
                                                <td style="width:50px;"><span>
                                                    @if($row->image)
                                                    <img src="{{url('public/clients/'.$row->image)}}" alt="user" width="50">
                                                ...
                                                <td>
                                                    <h6>{{$row->intake_name}}</h6><small class="text-muted">request to increase in payroll upto  {{$row->payroll}}</small><br><br><small class="text-muted">{{$row->time}}</small></td>
                                                <td><small class="text-muted">{{$row->date}}<br><br><br><br></small></td>
                                               <td>
                                                <a href="{{url('approvepayroll/'.$row->id)}}" class="" title=""> <img src="public/images/approve.png" alt="view" class="img-circle" style="height: 50px"></a>

                                            </td>
                                          <td>
                                                <a href="{{url('rejectpayrollreq/'.$row->id)}}"> <img src="public/images/remove.jpg" alt="remove" class="img-circle" style="height: 50px"></a>
