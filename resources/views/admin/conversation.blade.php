            ...
            <div class="card-header msg_head">
              <div class="d-flex msg_head">
                <div class="img_cont">
                  ...
                  @if($intakeco->image) 
                  <img src="{{url('public/clients/'.$intakeco->image)}}" alt="view" class="img-circle" style="height: 70px" width="70px">
                  @else
                ...
                <div class="user_info">
                  <span> {{$intakeco->intake_name}}</span>
<!--                   <p>1767 Messages</p>
 -->                </div>
              </div>
            </div>
            ...
            @if($intakeco->image) 
            <img src="{{url('public/clients/'.$intakeco->image)}}" alt="view" class="img-circle" style="height: 40px" width="40px">
            @else
...
<input type="hidden" name="token" id="token" value="{{ csrf_token() }}">
      <input type="hidden" name="intakeco_id" id="intakeco_id" value="{{$id}}">
...
<script>
    ...
    $('#send').on('submit',function(event){
        ...
        intakeco_id = $('#intakeco_id').val();

         $.ajax({
        headers: {
    'X-CSRF-TOKEN': "{{ csrf_token() }}"
    },     
          url: "{{ url('conversationMessage') }}",
          type:"POST",
          data:{
           token:token,      
           message:message, 
